...
def latent2code(model, source_upper, save_path="../codebook/result/inference/Trinity", prefix = 'train_codebook_bvh2upper_lower_root_again', w_grad=False):
    from VisualizeCodebook import main_2
    # print(source_upper.shape)       # (len, 64)
    recon, code, out_distance = main_2(source_upper, model, save_path, prefix, w_grad)
...
def inference(args, save_dir, save_name, wavlm_model, audio, sample_fn, model, n_frames=0, smoothing=False, SG_filter=False, minibatch=False, skip_timesteps=False, n_seed=seed, style=None, seed=123456, time_step=False, time=None, return_seed=False, w_grad=False, save_result=False):
    ...
    if minibatch:
        ...
        for i in range(0, num_subdivision):
            ...
            if w_grad:
                wavlm_feature.requires_grad = True
...
def inference_2(args, sample_fn, wavlm_model, vqvae_model, model, audionpy_path=None, audio_path=None, audio_wav=None, time_step=False, time=0, return_seed=False, w_grad=False, save_result=False):
    ...
    x, output_path, wavlm_feat, t_list = inference(args, save_dir, save_name, wavlm_model, audio_feat, sample_fn, model,
                                                   ...
                                                   skip_timesteps=time_step, style=style, seed=123456, time_step=time_step,
                                                   time=time,
    ...
    recon, code, distance = latent2code(vqvae_model, x_, None, None, w_grad)
    if w_grad:
    ...
    return y, code, wavlm_feat, distance, t_list


def single_inference(audio, config, sample_fn, wavlm_model, vqvae_model, diffusion_model, audio_wav=None, time_step=False, time=0, return_seed=False, w_grad=False, save_result=False):
    if not w_grad:
    ...
    if type(audio_wav) is np.ndarray:
        motion, code, wavlm_feat, distance, t_list = inference_2(config, sample_fn, wavlm_model, vqvae_model, diffusion_model,
    ...
    return motion, code, wavlm_feat, distance, t_list
...
def all_inference(ZEGGS_audio_fold, Trinity_audio_fold, ZEGGS_audio_fold_valid, Trinity_audio_fold_valid, config, sample_fn, wavlm_model, vqvae_model, diffusion_model, return_GT=False):
    ...
    for audio in train_audio_list:
        ...
        time = np.array([100, 200, 300, 400, 500, 600, 700, 800])        # [1, 1000]

        _, code, wavlm_feat, distance, t_list = single_inference(audio, config, sample_fn, wavlm_model, vqvae_model, diffusion_model, audio_wav=audio_wav, time_step=True, time=time, return_seed=True, w_grad=True)
...
if __name__ == '__main__':
    ...
    cd ./diffusion_latent/
    python sample.py --config=./configs/all_data.yml --train --no_cuda 3 --gpu 3
    '''

    # prefix = str(datetime.now().strftime('%Y%m%d_%H%M%S'))
    # save_dir = './result/sample_' + prefix
    save_dir = './result/Trinity'
    ...
    # './result/inference/new_DiffuseStyleGesture_model3_256/model001100000.pt'
    ...
    # audio = '../../trinity/audio/Recording_006.wav'     # "../dataset/ZEGGS/all_speech/067_Speech_2_x_1_0.npy"
    # _, code, _, distance, _ = single_inference(audio, config, sample_fn, wavlm_model, vqvae_model, diffusion_model, time_step=True, w_grad=False, save_result=True)      # for debug
    # pdb.set_trace()
    # motion (1, 112, len)  code (len//2, 1)
