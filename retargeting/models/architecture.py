...
class GAN_model(BaseModel):
    def __init__(self, args, character_names, dataset):
        ...
        self.n_topology = len(character_names)      # [['Talking_With_Hands'], ['Trinity']]
        self.models = []
        self.D_para = []
        self.G_para = []
        self.args = args

        for i in range(self.n_topology):        # 2
    ...
    def forward(self):
        ...
        # reconstruct
        for i in range(self.n_topology):        # 2
            motion, offset_idx = self.motions_input[i]          # [2, 99, 64], [0, 0]
            motion = motion.to(self.device)
            self.motions.append(motion)
            motion_denorm = self.dataset.denorm(i, offset_idx, motion)      # [2, 99, 64]
            self.motion_denorm.append(motion_denorm)
            offsets = [self.offset_repr[i][p][offset_idx] for p in range(self.args.num_layers + 1)]
            # num_layers    2   self.offset_repr  [[[1, 25, 3],[1, 90],[1, 180]],[]]    ->  [[2, 25, 3], [2, 90], [2, 180]]
            latent, res = self.models[i].auto_encoder(motion, offsets)      # -> [2, 112, 16], [2, 99, 64]
            res_denorm = self.dataset.denorm(i, offset_idx, res)        # -> [2, 99, 64]
            res_pos = self.models[i].fk.forward_from_raw(res_denorm, self.dataset.offsets[i][offset_idx])
            # self.dataset.offsets  [[1, 25, 3],[1, 25, 3]] ->  [2, 64, 25, 3]
            self.res_pos.append(res_pos)
            self.latents.append(latent)
            self.res.append(res)
            self.res_denorm.append(res_denorm)

            pos = self.models[i].fk.forward_from_raw(motion_denorm, self.dataset.offsets[i][offset_idx]).detach()       # -> [2, 64, 25, 3]
            ee = get_ee(pos, self.dataset.joint_topologies[i], self.dataset.ee_ids[i],
                        velo=self.args.ee_velo, from_root=self.args.ee_from_root)       # ee_velo   1   ee_from_root    1
            # self.dataset.joint_topologies
            # [(-1, 0, 1, 2, 3, 4, 0, 6, 7, 8, 9, 0, 11, 12, 13, 14, 15, 13, 17, 18, 19, 13, 21, 22, 23),
            #  (-1, 0, 1, 2, 3, 4, 0, 6, 7, 8, 9, 0, 11, 12, 13, 14, 15, 13, 17, 18, 19, 13, 21, 22, 23)]
            # self.dataset.ee_ids   [[5, 10, 16, 20, 24], [5, 10, 16, 20, 24]]
            # ee [2, 63, 5, 3]
            height = self.models[i].height[offset_idx]      # [[179.1899]], [0, 0] -> [[179.1899], [179.1899]], shape (2, 1)
            height = height.reshape((height.shape[0], 1, height.shape[1], 1))       # -> [2, 1, 1, 1]
            ee /= height
        ...
        for src in range(self.n_topology):
            for dst in range(self.n_topology):
                ...
                self.rnd_idx.append(rnd_idx)        # [0, 0]
                dst_offsets_repr = [self.offset_repr[dst][p][rnd_idx] for p in range(self.args.num_layers + 1)]     # ->  [[2, 25, 3], [2, 90], [2, 180]]
                fake_res = self.models[dst].auto_encoder.dec(self.latents[src], dst_offsets_repr)       # -> [2, 99, 64]
                fake_latent = self.models[dst].auto_encoder.enc(fake_res, dst_offsets_repr)     # -> [2, 112, 16]

                fake_res_denorm = self.dataset.denorm(dst, rnd_idx, fake_res)       # -> [2, 99, 64]
                fake_pos = self.models[dst].fk.forward_from_raw(fake_res_denorm, self.dataset.offsets[dst][rnd_idx])        # -> [2, 64, 25, 3]
                fake_ee = get_ee(fake_pos, self.dataset.joint_topologies[dst], self.dataset.ee_ids[dst],
                                 velo=self.args.ee_velo, from_root=self.args.ee_from_root)      # -> [2, 63, 5, 3]
                height = self.models[dst].height[rnd_idx]       # [[179.1899]], [0, 0] -> [[179.1899], [179.1899]], shape (2, 1)
                height = height.reshape((height.shape[0], 1, height.shape[1], 1))       # [2, 1, 1, 1]
    ...
    def backward_G(self):
        ...
        self.loss_G_total = 0
        for i in range(self.n_topology):
            rec_loss1 = self.criterion_rec(self.motions[i], self.res[i])        # [2, 99, 64]
            ...
            input_pos = self.motion_denorm[i][:, -3:, :] / height       # self.motion_denorm[i] [2, 99, 64] -> [2, 3, 64]
            ...
            # self.pos_ref [[2, 64, 25, 3], [2, 64, 25, 3]]  pos_ref_global   [2, 64, 25, 3]
        ...
        for src in range(self.n_topology):
            for dst in range(self.n_topology):
                cycle_loss = self.criterion_cycle(self.latents[src], self.fake_latent[p])       # [2, 112, 16], [2, 112, 16] ->
                self.loss_recoder.add_scalar('cycle_loss_{}_{}'.format(src, dst), cycle_loss)
                self.cycle_loss += cycle_loss
                ee_loss = self.criterion_ee(self.ee_ref[src], self.fake_ee[p])      # [2, 63, 5, 3], [2, 63, 5, 3] ->
    ...
    def optimize_parameters(self):
        ...
        # update Ds
        if self.args.gan_mode != 'none':    # lsgan
    ...
    def generate_bvh(self, result=None, mode=None):
        if mode == 'ZEGGS':
            dst_path = os.path.join(self.bvh_path, self.character_names[1][0])
            self.writer[1][0].write_raw(result, 'quaternion', os.path.join(dst_path, '{}_{}.bvh'.format(0, 0)))
        elif mode == 'Trinity':
            dst_path = os.path.join(self.bvh_path, self.character_names[0][0])
            self.writer[0][0].write_raw(result, 'quaternion', os.path.join(dst_path, '{}_{}.bvh'.format(0, 0)))

    def compute_test_result_global(self, lower=None, root=None, mode=None):
        p = 0
        if mode == 'ZEGGS':
            src = 0
        for dst in range(self.n_topology):
            for i in range(len(self.character_names[dst])):
                dst_path = os.path.join(self.bvh_path, self.character_names[dst][i])
                # (103, len)
                result = self.fake_res_denorm[p][i, ...].clone()
                if lower is not None:
                    pass
                else:
                    result[:40, :] = 0
                    result[-3:, :] = 0
                self.writer[dst][i].write_raw(result, 'quaternion',
                                              os.path.join(dst_path, '{}_{}.bvh'.format(self.id_test, src)))
            p += 1
    ...
    def latent2res(self, latents):
        ...
        #     pass
            ...
            # self.res_denorm.append(res_denorm)
            #
            # pos = self.models[i].fk.forward_from_raw(motion_denorm,
            #                                          self.dataset.offsets[i][offset_idx]).detach()  # -> [2, 64, 25, 3]
            # ee = get_ee(pos, self.dataset.joint_topologies[i], self.dataset.ee_ids[i],
            #             velo=self.args.ee_velo, from_root=self.args.ee_from_root)  # ee_velo   1   ee_from_root    1
            # # self.dataset.joint_topologies
            # # [(-1, 0, 1, 2, 3, 4, 0, 6, 7, 8, 9, 0, 11, 12, 13, 14, 15, 13, 17, 18, 19, 13, 21, 22, 23),
        ...
        for src in range(self.n_topology):
            for dst in range(self.n_topology):
                ...
                self.rnd_idx.append(rnd_idx)  # [0, 0]
                dst_offsets_repr = [self.offset_repr[dst][p][rnd_idx] for p in
                                    range(self.args.num_layers + 1)]  # ->  [[2, 25, 3], [2, 90], [2, 180]]
                fake_res = self.models[dst].auto_encoder.dec(self.latents[src], dst_offsets_repr)  # -> [2, 99, 64]
                fake_latent = self.models[dst].auto_encoder.enc(fake_res, dst_offsets_repr)  # -> [2, 112, 16]

                fake_res_denorm = self.dataset.denorm(dst, rnd_idx, fake_res)  # -> [2, 99, 64]
                fake_pos = self.models[dst].fk.forward_from_raw(fake_res_denorm,
                                                                self.dataset.offsets[dst][rnd_idx])  # -> [2, 64, 25, 3]
                fake_ee = get_ee(fake_pos, self.dataset.joint_topologies[dst], self.dataset.ee_ids[dst],
                                 velo=self.args.ee_velo, from_root=self.args.ee_from_root)  # -> [2, 63, 5, 3]
                height = self.models[dst].height[
                    rnd_idx]  # [[179.1899]], [0, 0] -> [[179.1899], [179.1899]], shape (2, 1)
                height = height.reshape((height.shape[0], 1, height.shape[1], 1))  # [2, 1, 1, 1]
                fake_ee = fake_ee / height
...
if __name__ == '__main__':
    ...
    print('OK')
    model = GAN_model(args, characters, dataset)
    model.is_train = False
    input_motion = [[torch.randn(1, 103, 64).to('cuda:5'), [0]], [torch.randn(1, 107, 64).to('cuda:5'), [0]]]      # (b, 112, len)
