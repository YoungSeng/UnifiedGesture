...
class Encoder(nn.Module):
    def __init__(self, args, topology):
        ...
        for i in range(args.num_layers):
            self.channel_base.append(self.channel_base[-1] * 2)     # [4, 8, 16]
        ...
        for i in range(args.num_layers):
            ...
            for _ in range(args.extra_conv):        # 0
    ...
    def forward(self, input, offset=None):
        ...
        if self.args.rotation == 'quaternion' and self.args.pos_repr != '4d':
            input = torch.cat((input, torch.zeros_like(input[:, [0], :])), dim=1)       # [1, 104, 64]

        for i, layer in enumerate(self.layers):
            if self.args.skeleton_info == 'concat' and offset is not None:
                self.convs[i].set_offset(offset[i])

            input = layer(input)         # [1, 104, 64] -> [1, 120, 32] -> [1, 112, 16]  4*26 -> 8*15 -> 16*7
        return input
...
class StaticEncoder(nn.Module):
    ...
    def forward(self, input: torch.Tensor):
        output = [input]
        for i, layer in enumerate(self.layers):
            input = layer(input)        # [1, 25, 3] -> [1, 90, 1] -> [1, 180, 1]
...
if __name__ == '__main__':
    ...
    topo = dataset.joint_topologies[0]      # len(topo) = 26
    edges = build_edge_topology(topo, torch.zeros((len(topo), 3)))      # len(edges) = 25
    args.is_train = False
    model = AE(args, edges)
    x = torch.rand(1, 103, 64)
    offset = [torch.rand(1, 26, 3), torch.rand(1, 90), torch.rand(1, 180)]
    y = model(x, offset)
