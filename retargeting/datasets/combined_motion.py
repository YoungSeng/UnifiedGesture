...
class MixedData(Dataset):
    ...
    def __init__(self, args, datasets_groups):
        ...
        all_datas = []
        for datasets in datasets_groups:    # [['ch01'], ['ch02']]
            offsets_group = []
            means_group = []
            vars_group = []
            ...
            for i, dataset in enumerate(datasets):
                ...
                mean = np.load('./datasets/' + args.dataset + '/mean_var/{}_mean.npy'.format(dataset))
                var = np.load('./datasets/' + args.dataset + '/mean_var/{}_var.npy'.format(dataset))
        ...
        for datasets in all_datas:      # [[<datasets.motion_dataset.MotionData object at 0x7f5879f06610>], [<datasets.motion_dataset.MotionData object at 0x7f57a1952a50>]]
            pt = 0
            motions = []
            skeleton_idx = []
            for dataset in datasets:
                ...
                pt += 1
            motions = torch.cat(motions, dim=0)     # ([4, 99, 64])
...
class TestData(Dataset):
    def __init__(self, args, characters):
        ...
        for i, character_group in enumerate(characters):
            ...
            for j, character in enumerate(character_group):
                ...
                new_offset = new_offset.reshape((1,) + new_offset.shape)
                offsets_group.append(new_offset)
                mean = np.load('./datasets/' + args.dataset + '/mean_var/{}_mean.npy'.format(character))
                var = np.load('./datasets/' + args.dataset + '/mean_var/{}_var.npy'.format(character))
                mean = torch.tensor(mean)
                mean = mean.reshape((1, ) + mean.shape)
                var = torch.tensor(var)
                var = var.reshape((1, ) + var.shape)
    ...
    def get_item(self, gid, pid, id):
        ...
        path = './datasets/' + self.args.dataset + '/{}/'.format(character)
