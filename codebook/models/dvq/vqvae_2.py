...
from dvqmodel.deepmind_enc_dec import DeepMindEncoder, DeepMindDecoder
from dvqmodel.openai_enc_dec import OpenAIEncoder, OpenAIDecoder
from dvqmodel.openai_enc_dec import Conv2d as PatchedConv2d
from dvqmodel.quantize import VQVAEQuantize, GumbelQuantize
from dvqmodel.loss import Normal, LogitLaplace
...
class VQVAE(pl.LightningModule):
    ...
    def forward(self, x):
        ...
        return x_hat, latent_loss + F.mse_loss(x, x_hat), ind
...
class VQVAE(nn.Module):
    def __init__(self, input_channels):
        ...
        self.decoder = Decoder(out_channels=input_channels)
        ...
        self.quantizer = QuantizerModule(input_channels//2, 512, 64)
    ...
    def forward(self, x):       # [8, 30, 112]
        z = self.encoder(x)     # [8, 56, 30, 1]
        z_q, latent_loss, ind = self.quantizer(z)       # [batch, 64, 30, 1]
        x_hat = self.decoder(z_q)
        return x_hat, latent_loss + F.mse_loss(x, x_hat), ind
...
if __name__ == "__main__":
    '''
    pip install attr, pytorch_lightning
    cd models
    python dvq/vqvae_2.py
    '''
    # cli_main()

    model_ = VQVAE(input_channels=16*7)
    x = torch.randn(128, 30, 16*7)
    a, b, c = model_(x)
    pdb.set_trace()
